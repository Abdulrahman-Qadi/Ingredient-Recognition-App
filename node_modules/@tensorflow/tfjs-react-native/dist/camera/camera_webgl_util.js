 ...
 */
export function drawTexture(gl, texture, dims, flipHorizontal, rotation) {
    const { program, vao, vertices, uniformLocations } = drawTextureProgram(gl, flipHorizontal, false, rotation);
...
export function runResizeProgram(gl, inputTexture, inputDims, outputDims, alignCorners, useCustomShadersToResize, interpolation, rotation) {
    ...
    if (!useCustomShadersToResize) {
        const textureFilter = interpolation === 'nearest_neighbor' ? gl.NEAREST : gl.LINEAR;
...
export function drawTextureProgram(gl, flipHorizontal, flipVertical, rotation) {
    ...
    if (!programCache.has(cacheKey)) {
        const vertSource = drawTextureProgramInfo.vertexShaderSource(flipHorizontal, flipVertical, rotation);
